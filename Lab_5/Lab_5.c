...
// Головна функція
int main(void){                                
    int temp;                     // Тимчасова змінна для запису відстаней
    int minindex;                 // Змінна для ітерацій циклу основного алгоритму
    int min;					  // Тимчасове значення довжини
    ...
    // Основний алгоритм
    do{
        minindex=10000;										 // Ставимо значення довжини 
        min=10000;											 // і місця мінімальної довжини великими числами
        for (int i=0; i<SIZE; i++){					 	     // Перебираємо масив відстаней 
        ...
        if(min!=10000){                                      // Якщо попередня умова виконалась
            for(int i=0; i<SIZE; i++){						 // Перебираємо рядок матриці
                if(Array[minindex][i]>0){					 // Якщо є звязок з вершиною
                    temp=min+Array[minindex][i];			 // Записуємо відстань в змінну temp
                    if(temp<distance[i]){					 // Якщо відстань менша, ніж теперішня відома відстань
    ...
    while (end > 0){ // Поки не дійшли до початкової вершини
        for(int i=0; i<SIZE; i++) // Переглядаємо всі вершини
            if (Array[end][i] != 0){   // Якщо зв'язок є
                ...
                if (temp == distance[i]){ // Якщо вага співпала з вирахуваною, значить, з цієї вершини був здійснений перехід
                    weight = temp;        // Зберігаємо нову вагу
                    end = i;        	  // Зберігаємо попередню вершину
                    ver[k] = i + 1; 	  // І записуємо її в масив
                    k++;				  // Збільшуємо прапорець для виводу 
                }
            }
    }
